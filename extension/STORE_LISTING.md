...
```
End-to-end encrypted P2P messages, overlaid on any page. Shared identity with the Closer Click ecosystem.
```
...
```
Closer Click Messenger is the Chrome extension companion to the Closer Click ecosystem (https://closer.click/). It lets you receive and reply to encrypted peer-to-peer messages on any page you visit, without switching tabs.
...
```
Display incoming end-to-end-encrypted messages from the Closer Click P2P network as an overlay on any page the user is currently visiting, and let the user reply without leaving that page.
```
