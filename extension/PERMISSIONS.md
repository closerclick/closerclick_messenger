...
Display incoming end-to-end-encrypted messages from the Closer Click P2P network as an overlay on any page the user is currently visiting, and let the user reply without leaving that page. Identity, contacts, and message history are loaded from the cross-origin Closer Click ecosystem (`messenger.closer.click`, `id.closer.click`, `store.closer.click`); the extension itself does not store user data.

## Permission justifications

### `offscreen`
Required to host the messenger PWA (`https://messenger.closer.click/`) inside an iframe in an offscreen document. That iframe maintains the WebSocket connection to the proxy server, performs the cryptographic handshakes and decryption, and persists threads to the shared `store.closer.click` origin. Without an offscreen document the extension cannot receive messages while no browser tab is visible.
...
### Host permission `https://messenger.closer.click/*`
...
### Host permission `https://id.closer.click/*`
...
### Host permission `https://store.closer.click/*`
The cross-instance message thread store. Same rationale as `id.closer.click`: the messenger PWA loads it as a sub-iframe to read/write threads that the user can also see in the web app.

### Host permission `wss://proxy.closer.click/*`
...
The extension itself does not store any user data. All persistence happens in the localStorage of the cross-origin domains (`id.closer.click` for keypairs and contacts, `store.closer.click` for message threads). No analytics, no telemetry, no third-party SDKs.
