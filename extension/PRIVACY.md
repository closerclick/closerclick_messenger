# Privacy Policy — Closer Click Messenger (Chrome Extension)
...
|---|---|---|
| Your identity keypairs (ECDSA P-256 for signing, ECDH P-256 for encryption) | `localStorage` of `https://id.closer.click/` (the shared identity vault, loaded inside an offscreen iframe) | Only you, on your device |
...
| Your messages in transit | Encrypted with AES-256-GCM and a per-message ECDH-derived key, then sent through `wss://proxy.closer.click` | Only the recipient (the proxy operator cannot decrypt) |
...
2. To also clear the shared identity vault: open `https://id.closer.click/`, run `localStorage.clear()` in the console.
